...
This helper creates labels + editable components for struct fields (`char`, `string`, scalar numeric/integer, numeric/logical arrays via `uitable`, `categorical`, `datetime`) and returns a function handle that reads back the updated struct values.
